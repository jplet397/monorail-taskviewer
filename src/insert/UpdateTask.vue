    ...
    methods: {
      ...
      updateTask() {
        this.$store.dispatch('updateTask', this.payload)
          .then((result) => {
            if (result === true) {
              this.$toasted.show(`Task ${this.payload.task} updated successfully`, { duration: 5000 });
              this.$router.push('/');
            } else {
              this.$toasted.show(`Failed to update Task ${this.payload.task}, Msg: ${result}`, { duration: 5000 });
            }
          });
      },
      getPayLoad() {
        ...
        this.$store.dispatch('getTasksById', anId)
          .then((task) => {
            if (task !== false) {
              ...
              this.payload.trolley = task.TrolleyId;
              this.payload.location = task.CurrentLocation.LocId;
              this.payload.source = task.SourceLoc.LocId;
